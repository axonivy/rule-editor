...
export async function start() {
  ...
  const reconnect = async (connection: Connection, oldClient: RuleClientJsonRpc) => {
    await oldClient.stop();
    return initialize(connection);
  };

  await webSocketConnection<RuleClientJsonRpc>(RuleClientJsonRpc.webSocketUrl(server)).listen({
