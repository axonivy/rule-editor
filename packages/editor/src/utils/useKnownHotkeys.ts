...
export const useKnownHotkeys = () => {
  ...
  const addRule = useMemo<KnownHotkey>(() => {
    const hotkey = 'A';
    return { hotkey, label: t('hotkey.addRule', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const deleteRule = useMemo<KnownHotkey>(() => {
    const hotkey = 'Delete';
    return { hotkey, label: t('hotkey.deleteRule', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
