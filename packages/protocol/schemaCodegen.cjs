...
var schemaUri =
  'https://jenkins.ivyteam.io/job/core_json-schema/job/simple-rule-engine/lastSuccessfulBuild/artifact/build/schema/target/editor-ts/14.0/rule-tsgen.json';
...
if (schemaUri.startsWith('http')) {
  loadJson(schemaUri)
    .then(schema => tsGen.compile(schema, 'roles'))
    .then(ts => writeSrc(ts));
} else {
