...
const tsOut = path.resolve('./src/data/rule.ts');
var schemaUri =
  'https://jenkins.ivyteam.io/job/core_json-schema/job/master/lastSuccessfulBuild/artifact/workspace/ch.ivyteam.ivy.dialog.form.lsp.schema/target/schema/form/13.2.3/rule-tsgen.json';
...
if (schemaUri.startsWith('http')) {
  loadJson(schemaUri)
    .then(schema => tsGen.compile(schema, 'forms'))
    .then(ts => writeSrc(ts));
} else {
